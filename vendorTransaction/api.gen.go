...
func (c *Client) GetTransaction(ctx context.Context, transactionId string) (*http.Response, error) {
	req, err := NewGetTransactionRequest(c.Endpoint, transactionId)
	if err != nil {
		return nil, err
	}
...
func NewGetTransactionRequest(endpoint string, transactionId string) (*http.Request, error) {
	var err error
	...
	basePath := fmt.Sprintf("/vendor/transactions/v1/transactions/%s", pathParam0)
	if basePath[0] == '/' {
		basePath = basePath[1:]
	}
